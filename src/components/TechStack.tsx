            ...
            <div className="grid grid-cols-3 lg:grid-cols-6 gap-4">
              {lifecycle.map(({ label, note, Icon }, i) => (
                <div key={label} className="relative flex flex-col items-center text-center">
                  <div
                    className="relative z-10 w-16 h-16 flex items-center justify-center mb-4 border bg-bg"
                    style={{
                      borderColor: i === 0 || i === lifecycle.length - 1
                        ? 'rgba(200,16,46,0.35)'
                        : '#E0DBD5',
                      color: i === 0 || i === lifecycle.length - 1
                        ? '#C8102E'
                        : '#8A8070',
                    }}
                  >
                  ...
                  <p className="text-[11px] text-ink font-medium mb-0.5">{label}</p>
                  <p className="text-[10px] text-muted font-mono">{note}</p>
                  {i < lifecycle.length - 1 && (
                    <div className="absolute top-[30px] left-[calc(50%+36px)] text-dim text-[10px] hidden lg:block select-none">
                      →
                    </div>
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {columns.map((col, ci) => (
            <FadeIn key={col.label} delay={ci * 80 + 140}>
              <div>
                <p className="text-[10px] text-red/65 tracking-[0.2em] uppercase font-medium mb-6 pb-4 border-b border-line">
                  {col.label}
                </p>
                <div className="space-y-6">
                  {col.items.map((item) => (
                    <div key={item.name}>
                      <p className="text-sm text-ink font-medium mb-1.5">{item.name}</p>
